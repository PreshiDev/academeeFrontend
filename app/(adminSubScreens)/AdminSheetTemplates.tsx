...
const UploadSheetTemplate = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const response = await api.post('/reportsheet/', formData, {
        headers: {
      ...
      Alert.alert('Success', 'Report Sheet uploaded successfully');
      reset();
      setPdf(null);
      setPdfName('');
      setStatus('');
  ...
  return (
    <View style={styles.container}>
      <Appbar.Header>
        <Appbar.Content title="Upload Report Sheet" />
      </Appbar.Header>

      <Card style={styles.card}>
        <Card.Title title="Upload Report Sheet" />
        <Card.Content>
          <Controller
            control={control}
            name="title"
      ...
      >
        Report Sheet uploaded successfully!
      </Snackbar>
    </View>
  );
};
...
export default UploadSheetTemplate;