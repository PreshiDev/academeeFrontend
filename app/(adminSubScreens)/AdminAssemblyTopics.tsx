...
const UploadAssemblyTopics = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const response = await api.post('/assemblytopic/', formData, {
        headers: {
      ...
      Alert.alert('Success', 'Assembly Topic uploaded successfully');
      reset();
      setPdf(null);
      setPdfName('');
      setStatus('');
  ...
  return (
    <View style={styles.container}>
      <Appbar.Header>
        <Appbar.Content title="Upload Assembly Topics" />
      </Appbar.Header>

      <Card style={styles.card}>
        <Card.Title title="Upload Assembly Topics" />
        <Card.Content>
          <Controller
            control={control}
            name="title"
      ...
      >
        Assembly Topic uploaded successfully!
      </Snackbar>
    </View>
  );
};
...
export default UploadAssemblyTopics;